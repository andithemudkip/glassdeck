...
Sub-analysis 17 (`540 D1` full discriminator scan) — done. **The "warm-up index" name is now fully retracted; new best interpretation is a slow-updated ECU fuel/injection-state lookup.** Seven discriminators on the 9 521-point corpus in [`scripts/first_moving_ride_d1_full_scan.py`](../../scripts/first_moving_ride_d1_full_scan.py) and [`scripts/first_moving_ride_d1_step_response.py`](../../scripts/first_moving_ride_d1_step_response.py):

- **Time constant ≈ 1 s.** `r(D1(t), grip(t+lag))` peaks at lag +1000..+1200 ms in every one of the 5 files (r 0.90–0.96). D1 does not track instantaneous grip — step-response confirms it latches for 500–1000 ms during rapid grip transitions.
- **Overrun decay is stepwise toward the idle floor.** In a 4.5 s clean overrun, D1 walked 50 → 24 → 14 → 19 → 15 → 14 (LSB), i.e. from the pre-close cruise value down to the coolant-keyed idle lookup value. Not a lookup snapping to (RPM, throttle, coolant) — a slow-updated integrator or ~1 Hz state variable.
- **(grip, RPM, coolant) fully determines D1.** OLS `D1 ≈ 5.92 + 0.4055·grip + 0.00921·RPM − 0.1375·coolant` explains 73 % of variance. Residuals correlate at |r| < 0.07 against `121_A`, `|121_A|`, `max(0, 121_A)`, `121_B`, `rear_kmh`, `gear`, and their RPM products — **nothing outside those three adds information**.
- **Gear-invariant under drive.** At 4500-5000 RPM / 20-60 raw grip: D1 ≈ 49-51 across gears 3/4/5 despite vehicle speed differing by 22 km/h (i.e. very different real load at the wheel). Rules out any measured-load quantity (MAP, MAF, air-mass, torque).
- **Overrun spike scales with `121_A` magnitude.** Overrun D1 by `121_A` bin: strong braking (-40..-30) → D1 μ 59, light braking (-10..0) → D1 μ 19. The elevation is a decel-severity compensation, not a load measurement.
- **Ride-integrated fuel sanity check.** Mean(D1) × 952 s of engine-on = 39 842 LSB·s vs 326 mL rider fuel (3.4 L/100km × 9.58 km) implies 8.18 μL per LSB·s. Predicts idle burn 0.41 L/hr, 5000-RPM cruise 1.6 L/hr, WOT 4.4 L/hr — all in the right ballpark for a 390 cc single. Better than the torque model in that D1's nonzero idle floor removes the need for a separate `f_idle`.

Top candidate for the physical quantity: **filtered fuel/injection index** (running-average setpoint). Ranked second: base ignition-timing map output. Rejected: any measured-load quantity, rider grip mirror, butterfly command angle. File rename to `signal-540-d1-fuel-index` is warranted only after a key-cycle transition capture and a long-overrun coast-down disambiguate integrator-vs-lookup. [[signal-fuel-injection-setpoint]] rewritten with the new blockquote at the top.
