...
Bit 0 toggles uniformly across all 3 idle runs (~80 toggles), but **bit 1 toggles fall as coolant rises** (80 cold → 63 warm → 8 op-temp) and **bits 2-4 stop toggling entirely by Run 3**. Exactly what a coolant-binned encoding would do: the high bits flip during warm-up and stabilise at operating temperature; the low bits keep flipping with noise/jitter. Independent corroboration that [[signal-coolant-derived]] is real and not coincidence.
...
- ✅ Update [[signal-coolant-derived]] with the bit-level corroboration.
