...
| `129` | D0   | SINGLE-CAUSE(gear) — known  | **side observation that turned into a finding**: distinct=5 in gear session, not 2. Values `0x00,0x10` are N/1st as expected, but `0x08,0x0A,0x18` appear transiently. Follow-up alignment of bit-3 / bit-1 vs event marks established [[signal-shift-lever]] — lo nibble carries shift-lever sensor state (bit 3 = lever displaced, bit 1 = failed-shift flag). | new finding [[signal-shift-lever]]; updated [[signal-gear-position]] |
