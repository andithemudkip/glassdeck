...
**Last touched:** 2026-07-09 — wifi-bridge M4 + M7a code-complete (on-device timestamps, gap-fill ring, browser-side OPFS capture, mark endpoint). Bench verification against the bike is the next step; see [`firmware/wifi-bridge/README.md`](../firmware/wifi-bridge/README.md) § Current state.
...
- **Firmware.** `can-logger` v1 shipped, listen-only @ 500 kbps. `wifi-bridge` M1–M4 + M6 + M7a shipped; M7b (mark button UI) deferred to M8. Shared `firmware/lib/{twai,slcan,status_led}/` used by both targets. Iteration: OTA reflash via `POST /ota`; live capture via `websocat -n ws://192.168.4.1/stream | scripts/capture.py --stdin`.
...
1. **Bench-verify wifi-bridge M4 + M7a** against the bike. Ring overflow test (60 s WiFi disconnect → exactly one `# GAP <ms>` marker). Sequence in [`firmware/wifi-bridge/README.md`](../firmware/wifi-bridge/README.md) § Current state.
