...
**Last touched:** 2026-07-23 — moving-corpus follow-up analyses on `540 D1` and `121 A`. Two promotions: [[signal-fuel-injection-setpoint]] (renamed from `warmup_index`; base fuel-injection setpoint, ~1 Hz ECU recompute, cold-start 3-stage enrichment shape, still `provisional` pending coast-down for exact scaling) and [[signal-engine-torque]] (renamed from `byte-121-twin-int16`; **promoted to `confirmed`** on the strength of ignition-cut collapse, sign-flip drive/overrun, physics-based LSB order-of-magnitude check, and twin-channel safety-monitor redundancy — LSB refinement still open pending coast-down). See [[2026-07-22-first-moving-ride]] sub-analysis 17 for the discriminator scan.
