...
**Status:** milestones 1–6 + M7a code-complete (compile-verified, bench check pending hardware plug-in). Milestone 4 (gap-fill ring + browser-side OPFS capture) landed alongside the firmware-side of M7 (`POST /mark`) — see M4/M7a below. M7b (mark button UI) and M8 (dev view + Active-unknown-bytes port) have been dropped; the operator-facing surface is being redirected into the signal-discovery wizard at `/discover` per [ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md). Free-form marks continue to work via `curl -X POST /mark` from a nearby laptop. See § Milestones for what's staged, § Current state for the next step.
...
- [ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md) — signal-discovery wizard at `/discover`, dynamic `GET`/`POST /signals` with NVS `user_signals` blob. Adds the second embedded HTML page + typed procedure schema v2.
...
Split from the original M7: the endpoint landed with M4 because it writes into the same ring. The **UI** side — a mark button + label input in the browser — was originally slated for M8's dev view but has since been dropped along with M8 itself (see [ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md)). Procedure-driven captures under the discovery wizard insert marks automatically at step boundaries; free-form marks are covered by `curl -X POST /mark` from a nearby laptop. The rider view stays glanceable-instrument territory. See also [[experiment-design-hand-driven-marks]] — riders often can't press marks in real time anyway.
...
M8 was originally scoped as a `/dev` view (raw-frame ticker relocated from M5, Active-unknown-bytes port from `scripts/live_view/`, mark button UI). Dropped in favor of redirecting the operator-facing browser surface into the signal-discovery wizard at `/discover` — see [ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md), which supersedes [ADR 0009](../../docs/decisions/0009-bike-profile-extraction-wizard.md). Rationale: porting `scripts/live_view/` to the browser duplicates existing R&D capability without opening a new user segment; the wizard opens the zero-Python bike-portability path that ADR 0009 committed the project to. `scripts/live_view/` stays as the R&D deep-dive TUI. The wizard is tracked as its own milestone stream under ADR 0019 rather than a wifi-bridge milestone, since it spans procedure schema, browser UX, and firmware NVS work.
...
Beyond bench verification, the next work stream on this firmware is the signal-discovery wizard from [ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md) — a second embedded page at `/discover`, typed procedure schema v2, and (in later phases) NVS `user_signals` + dynamic `GET`/`POST /signals`. Phase 1 (capture-only browser runner, Python analyzer) doesn't need hardware beyond what M6 already needs. Actual on-bike ride captures still wait on the F7 power perfboard, but that's a validation-scope question.
...
- **`scripts/live_view/` retirement.** Previously flagged as a long-arc goal via a `/dev` browser port. That path is dropped ([ADR 0019](../../docs/decisions/0019-browser-signal-discovery-wizard.md)) — the TUI is not being ported. The browser surface owns the *rider* view and the *wizard* (`/discover`); the TUI owns R&D deep dives (BaselineStats z-scores, watch pins with sparklines, hypothesis / expect-shape workflow, authored `.procedure.yaml` files). Two tools for two users, no retirement plan for either.
